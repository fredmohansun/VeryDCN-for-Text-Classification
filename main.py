...
train_x_path = dict(zip(Datasets, list(map(joiner,['preprocessed_data/']*len(Datasets),Datasets,['_train.txt']*len(Datasets)))))
train_y_path = dict(zip(Datasets, list(map(joiner,['preprocessed_data/']*len(Datasets),Datasets,['_train_labels.txt']*len(Datasets)))))
test_x_path = dict(zip(Datasets, list(map(joiner,['preprocessed_data/']*len(Datasets),Datasets,['_test.txt']*len(Datasets)))))
test_y_path = dict(zip(Datasets, list(map(joiner,['preprocessed_data/']*len(Datasets),Datasets,['_test_labels.txt']*len(Datasets)))))
...
parser.add_argument('--downsample', type=int, choices=[0,1,2,3], help='0: No, 1: maxpool, 2: Resnet, 3(default): Kmax')
...
downsample = 3 if args.downsample is None else args.downsample
...
for inputs, label in test_loader:
    ...
    running_loss += loss.data.item()
    counter += batch_size
print(basic_message.format(epoch, '\"validation\"', running_acc/counter*100.0, running_loss/(counter/batch_size),float(time.time()-timer1)))
